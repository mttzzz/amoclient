...
class Deleter
{
     ...
     *
     * Не проверено: что отвечает амо на воронку, которую удалить НЕЛЬЗЯ —
     * основную либо непустую. Если тот же код, отличить по ответу будет нечем,
     * и неоднозначность останется амошной, как у сделок с «Недостаточно прав».
     * Поэтому `false` здесь означает «не подтверждено», а не «удалено».
     */
