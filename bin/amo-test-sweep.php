...
/*
 * Ненулевой код возврата на failed: свип — сетка, и её молчаливое «ну не
 * смогло» ровно то, ради чего сетку ставили. Warnings кодом не поднимаем —
 * это состояние аккаунта (отключённые покупатели, упёршийся в потолок скан),
 * а не поломка уборки; печатаются они всё равно.
 */
exit($report['failed'] === [] ? 0 : 1);
