 ...
 *
 * ЗАПУСКАТЬ ТОЛЬКО В ОДИН ПРОЦЕСС, ПОСЛЕДОВАТЕЛЬНО. Не распараллеливать ради
 * скорости, не гонять одновременно с тестовым прогоном. Документированный
 * лимит amo — 7 запросов в секунду на интеграцию (50 на аккаунт); превышение
 * даёт 429, а ПОВТОРНЫЕ нарушения блокируют аккаунт целиком: на любой запрос в
 * API начинает возвращаться 403. Пострадает при этом не свип и не тесты, а
 * рабочие интеграции клиента на этом же аккаунте. Отсюда же пауза между
 * удалениями внутри AmoTestSweeper — распараллеливание её обнуляет.
 */
...
/*
 * Ненулевой код возврата на failed и stale. failed — очевидно: свип сетка, и
 * её молчаливое «ну не смогло» ровно то, ради чего сетку ставили. stale — тот
 * же класс, но менее заметный: amo ответил «сущности уже нет» о том, что свип
 * секунду назад видел в обычной выборке, то есть уборка, возможно, не
 * состоялась, а выглядит состоявшейся.
 *
 * Warnings кодом не поднимаем — это состояние аккаунта (отключённые
 * покупатели, упёршийся в потолок скан), а не поломка уборки; печатаются они
 * всё равно.
 */
exit($report['failed'] === [] && $report['stale'] === [] ? 0 : 1);
