...
final class AmoTestSweeper
{
    ...
    /*
     * Размер страницы. Официальный потолок выборки amo — 250 сущностей за
     * запрос (§9), это поднимает охват дискавери с 1500 до 2500 записей на тип
     * без единого лишнего запроса — самое дешёвое из доступных смягчений риска
     * «наш хвост за потолком страниц».
     *
     * ВАЖНО: AbstractModel::limit() сегодня схлопывает всё выше 150, то есть
     * фактическая страница пока меньше запрошенной. Поэтому конец выборки
     * определяется по РАЗМЕРУ ПЕРВОЙ СТРАНИЦЫ, а не по этой константе: сравнив
     * с константой, свип на первой же клампнутой странице решил бы, что она
     * неполная, и читал бы ровно одну страницу на тип. Как потолок в src
     * поднимут, здесь не меняется ничего.
     */
    private const SCAN_PAGE_LIMIT = 250;
