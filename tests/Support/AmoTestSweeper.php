...
final class AmoTestSweeper
{
    ...
    /*
     * В корзину: запись остаётся в аккаунте, скрытая из обычных выборок; purge
     * недоступен нигде. Достоверный признак «в корзине» — отсутствие в обычной
     * выборке при наличии в only_deleted, а НЕ флаг is_deleted: у контактов и
     * компаний он стоит false даже для удалённых (§9.6), в отличие от сделок.
     * Само поле поэтому нигде в свипе не используется.
     */
     ...
     *
     * leads/contacts/companies — TRASHED, и это тоже доказано замером, а не
     * аналогией: §7.3/§8.5/§9.6 — запись пропадает из обычной выборки и
     * достаётся через only_deleted. Флаг is_deleted при этом доверия не
     * заслуживает: у контактов и компаний он false даже у удалённых.
     *
    ...
    /* Страниц на скан — потолок, чтобы свип не превращался в выкачивание аккаунта. */
    private const MAX_PAGES = 10;

    /*
     * Размер страницы. 150, а НЕ 250 — и это не про границу API: замером она
     * ровно 250 (251 amo тихо срезает), но на объёмных выдачах при 250
     * наблюдались падения амо (производственный опыт владельца; косвенно
     * подтверждается официальной рекомендацией уменьшать размер запроса при
     * 504). Свип ходит по сделкам с кастомными полями и вложенными
     * сущностями — по самым тяжёлым выдачам, какие тут бывают.
     *
     * Размен несимметричен: лишние страницы стоят секунд, падение на середине
     * скана — это незавершённая уборка в боевом аккаунте.
     *
     * Конец выборки определяется по РАЗМЕРУ ПЕРВОЙ ОТДАННОЙ СТРАНИЦЫ, а не по
     * этой константе. Сравнение с константой ломалось бы от любого расхождения
     * запрошенного и фактического (кламп в AbstractModel::limit(), тихий срез
     * на стороне amo): первая же «неполная» страница читалась бы как конец
     * коллекции, и свип брал бы ровно одну страницу на тип — с нормальным
     * отчётом. Поэтому смена этого числа в обе стороны безопасна.
     */
    private const SCAN_PAGE_LIMIT = 150;

    /*
     * Пауза между запросами на удаление. Документированный лимит amo — 7
     * запросов в секунду на интеграцию (§9), а часть типов сносится строго по
     * одному (каталоги, воронки, задачи, примечания — списочной формы у их
     * роутов нет), то есть свип генерирует ровно N запросов подряд.
     *
     * 200 мс, а не 150: 150 мс это 6,7 запроса в секунду при НУЛЕВОЙ сетевой
     * задержке, то есть весь запас до потолка держался бы на латентности сети.
     * Цена ошибки несоразмерна экономии: повторное превышение блокирует
     * аккаунт целиком, и 403 начинает получать не наш свип, а рабочие
     * интеграции клиента.
     */
    private const DELETE_THROTTLE_US = 200000;
     ...
     *
     * Фильтровать уже удалённых здесь не нужно и НЕ НАДО добавлять по флагу
     * is_deleted: обычная выборка удалённых не отдаёт вовсе (§8.5), а сам флаг
     * врёт — у контактов и компаний он false и у лежащих в корзине (§9.6).
     * Проверка «в корзине ли» по нему дала бы неверный ответ на двух типах из
     * трёх, причём молча.
     */
    ...
    private function sweepNotesAndCalls(AmoClientOctane $amo, int $from, int $to): void
    {
        ...
        foreach ($collections as $parent => $notes) {
            $windowed = $notes->filterUpdatedAt($from, $to)->orderByUpdatedAtDesc();
        ...
        try {
            ...
            for ($page = 1; $page <= self::MAX_PAGES; $page++) {
                ...
                if (self::repeatsPreviousPage($chunk, $previousChunk)) {
                    if (self::repeatedPageMayHideRecords($chunk)) {
    ...
    /**
     * Может ли за повторившейся страницей лежать недостижимое.
     *
     * Меряем относительно ЗАПРОШЕННОГО размера, а не отданного, и это ровно то
     * место, где сломался стык двух верных по отдельности решений. Конец
     * коллекции определяется по фактическому размеру страницы (иначе клампнутая
     * первая страница читалась бы как последняя), но здесь вопрос другой: «мог
     * ли amo отдать больше, чем отдал». Это утверждение о запросе. На роуте,
     * который игнорирует и page, и limit, фактический размер равен сам себе
     * всегда, и сравнение с ним объявляло дырой в уборке нормальную выдачу — а
     * предупреждение, которое горит каждый прогон, оператор перестаёт читать, и
     * настоящее проходит мимо.
     *
     * Остаточная неточность признаётся: коллекция ровно в SCAN_PAGE_LIMIT
     * записей на роуте, игнорирующем page, даст предупреждение без причины.
     * Отличить её от усечённой можно только лишним запросом, а цена ошибки
     * несимметрична — лучше лишний раз спросить, чем молча потерять хвост.
     *
     * @param  array<mixed>  $chunk
     */
    private static function repeatedPageMayHideRecords(array $chunk): bool
    {
        return count($chunk) >= self::SCAN_PAGE_LIMIT;
    }
    ...
    public static function render(array $report): string
    {
         ...
         */
        $out[] = 'Отправлено в корзину (скрыто из обычных выборок, purge недоступен нигде):';
