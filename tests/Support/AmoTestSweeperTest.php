...
class AmoTestSweeperTest extends TestCase
{
    ...
    /**
     * Стык двух верных по отдельности решений, на котором свип соврал в
     * боевом прогоне: конец коллекции меряется по ФАКТИЧЕСКОМУ размеру
     * страницы, а «мог ли amo отдать больше» — по ЗАПРОШЕННОМУ. На роуте,
     * который игнорирует и page, и limit, фактический размер равен сам себе
     * всегда, и предупреждение о недостижимых записях горело на нормальной
     * выдаче из сорока воронок.
     */
    public function test_repeated_short_page_is_the_whole_collection(): void
    {
        $mayHide = new ReflectionMethod(AmoTestSweeper::class, 'repeatedPageMayHideRecords');

        /* Сорок воронок при запрошенных 150 — это вся коллекция, не усечение. */
        $this->assertFalse($mayHide->invoke(null, array_fill(0, 40, ['id' => 1])));

        /* Полная страница при проигнорированном page — за ней может лежать недостижимое. */
        $this->assertTrue($mayHide->invoke(null, array_fill(0, 150, ['id' => 1])));
    }
