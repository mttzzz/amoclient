...
- Целевая версия либы после всей эпопеи — `4.0`. **Публичный API менять МОЖНО** (решение владельца, 22.07.2026): потребителя всего два — `octane` и `masterm`, оба свои и обновляются вместе с либой, хвостов несовместимости не остаётся. Нужен рефакторинг — делаем в либе, затем правим потребителей. Требование не «не менять», а «менять осознанно»: изменение поведения должно быть намеренным, задокументированным и не маскирующим ошибки (гард, превращающий нештатный ответ amo в тихий пустой массив, — дефект независимо от свободы менять API).
...
- Create/Modify: `src/Models/*`/`src/Entities/*` — удаление реализуем **методами библиотеки** (`delete()`), а не сырыми ajax-строками в тестах. Публичный API менять разрешено (см. Global Constraints), поэтому это не исключение, а предпочтительный путь: потребители (`octane`, `masterm`) получают удаление как штатную возможность либы.
...
- [ ] Изменения публичного API (если были) перечислены в отчёте SP0 — чтобы потребители `octane` и `masterm` были обновлены следом.
